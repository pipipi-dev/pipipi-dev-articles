...
1. **Will IDs be exposed in URLs?** → Avoid sequential IDs if exposed
2. **Need time-based sorting?** → Use UUID v7 or ULID
3. **Is write performance critical?** → Sequential IDs for large datasets
4. **Is ID length important?** → NanoID or CUID2 for URLs
